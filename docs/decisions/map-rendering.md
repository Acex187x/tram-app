...
| band | zoom | rendering |
|---|---|---|
| 1 | < 13.2 | PID-red dots (`tram-dots`) |
| 2 | 13.2–14.8 | badge circle + line-number `SymbolLayer` |
| 3–4 | ≥ 14.8 | articulated 3D `ModelLayer` (sections source) |

`BAND_DOTS_TO_BADGES = 13.2`, `BAND_BADGES_TO_MODELS = 14.8`, `BAND_FADE = 0.3`
(crossfade half-width). Night lines 90–99 render dark-blue (`Tram.night`) instead
of PID red via the `BADGE_COLOR` expression (`TramLayers.tsx:112`).
...
3. **v2 (behind-view, gesture-persistent follow, banner-only stop).** Default
   ...
   - In **live** position mode the camera tracks the raw observed bearing/position
     rather than the sim (`TramLayers.tsx:256`).
