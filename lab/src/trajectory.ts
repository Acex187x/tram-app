...
export function modalReleaseMs(
  ...
  // Парадокс инспекции (ночной вердикт 2026-08-22): Normal-модель релиза при
  // стоянии сильно ЗА пределами распределения говорит «наверняка уехал» — но
  // наблюдаемое стояние это прямая улика обратного: трамвай в тайминг-холде
  // (ночное выравнивание графика, 2–5 мин; доля стоянок >120 с ночью ×20).
  // За mean+2σ переключаемся на хвостовую эвристику: ожидаемый остаток холда
  // растёт с уже отстоянным (Линди), кап 180 с — модель перестаёт уезжать от
  // доказуемо стоящего трамвая, а любой сдвиг фикса всё равно ре-анкорит.
  if (standingS > mean + 2 * sd) {
    return Math.round(t0Ms + Math.min(180, standingS * 0.5) * 1000);
  }
